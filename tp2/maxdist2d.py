...
def max_dist():
    ...
    proms = [0,0,0,0,0,0]
    min = max_dist_cells30[0]
    max = 0
    for n in max_dist_cells30:
        proms[0] += n
        if n < min:
            min = n
        if n > max:
            max = n
    proms[0] = proms[0]/len(max_dist_cells30)
    minmax30 = (proms[0]-min, max-proms[0])

    min = max_dist_cells50[0]
    max = 0
    for n in max_dist_cells50:
        proms[1] += n
        if n < min:
            min = n
        if n > max:
            max = n
    proms[1] = proms[1]/len(max_dist_cells50)
    minmax50 = (proms[1]-min, max-proms[1])

    min = max_dist_cells70[0]
    max = 0
    for n in max_dist_cells70:
        proms[2] += n
        if n < min:
            min = n
        if n > max:
            max = n
    proms[2] = proms[2]/len(max_dist_cells70)
    minmax70 = (proms[2]-min, max-proms[2])

    min = max_dist_cells80[0]
    max = 0
    for n in max_dist_cells80:
        proms[3] += n
        if n < min:
            min = n
        if n > max:
            max = n
    proms[3] = proms[3]/len(max_dist_cells80)
    minmax80 = (proms[3]-min, max-proms[3])

    min = max_dist_cells90[0]
    max = 0
    for n in max_dist_cells90:
        proms[4] += n
        if n < min:
            min = n
        if n > max:
            max = n
    proms[4] = proms[4]/len(max_dist_cells90)
    minmax90 = (proms[4]-min, max-proms[4])

    min = max_dist_cells100[0]
    max = 0
    for n in max_dist_cells100:
        proms[5] += n
        if n < min:
            min = n
        if n > max:
            max = n
    proms[5] = proms[5]/len(max_dist_cells100)
    ...
    for n in max_dist_cells30:
        ...
        if n > max:
            max = n
    promsnew[0] = promsnew[0]/len(max_dist_cells30)
    ...
    for n in max_dist_cells50:
        ...
        if n > max:
            max = n
    promsnew[1] = promsnew[1]/len(max_dist_cells50)
    ...
    for n in max_dist_cells70:
        ...
        if n > max:
            max = n
    promsnew[2] = promsnew[2]/len(max_dist_cells70)
    ...
    for n in max_dist_cells80:
        ...
        if n > max:
            max = n
    promsnew[3] = promsnew[3]/len(max_dist_cells80)
    ...
    for n in max_dist_cells90:
        ...
        if n > max:
            max = n
    promsnew[4] = promsnew[4]/len(max_dist_cells90)
    ...
    for n in max_dist_cells100:
        ...
        if n > max:
            max = n
    promsnew[5] = promsnew[5]/len(max_dist_cells100)
    ...
    for n in max_dist_cells30:
        ...
        if n > max:
            max = n
    promsnewb[0] = promsnewb[0]/len(max_dist_cells30)
    ...
    for n in max_dist_cells50:
        ...
        if n > max:
            max = n
    promsnewb[1] = promsnewb[1]/len(max_dist_cells50)
    ...
    for n in max_dist_cells70:
        ...
        if n > max:
            max = n
    promsnewb[2] = promsnewb[2]/len(max_dist_cells70)
    ...
    for n in max_dist_cells80:
        ...
        if n > max:
            max = n
    promsnewb[3] = promsnewb[3]/len(max_dist_cells80)
    ...
    for n in max_dist_cells90:
        ...
        if n > max:
            max = n
    promsnewb[4] = promsnewb[4]/len(max_dist_cells90)
    ...
    for n in max_dist_cells100:
        ...
        if n > max:
            max = n
    promsnewb[5] = promsnewb[5]/len(max_dist_cells100)
    ...
    time1 = np.arange(0, len(ev_cells30new), 1)
    time2 = np.arange(0, len(ev_cells50new), 1)
    time3 = np.arange(0, len(ev_cells70new), 1)
    time4 = np.arange(0, len(ev_cells80new), 1)
    time5 = np.arange(0, len(ev_cells90new), 1)
    time6 = np.arange(0, len(ev_cells100new), 1)

    plt.plot(time1, ev_cells30new, label='30%')
    plt.plot(time2, ev_cells50new, label='50%')
    plt.plot(time3, ev_cells70new, label='70%')
    plt.plot(time4, ev_cells80new, label='80%')
    plt.plot(time5, ev_cells90new, label='90%')
    plt.plot(time6, ev_cells100new, label='100%')
    plt.legend()
    plt.xlabel('Tiempo (s)')
    plt.ylabel('Distancia max de celda viva al origen (m)')
    plt.show()
